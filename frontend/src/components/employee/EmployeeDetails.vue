...
<template>
  <div class="site-padding">
    <div class="grid py-6 gap-6 md:grid-cols-2">
      ...
      <div class="mb-auto md:order-2 mx-auto text-center relative group">
        <div
          class="bg-primary-100/20 h-96 w-full absolute -bottom-5 rounded-xl -z-10 group-hover:translate-y-1 transition-transform duration-200"
        ></div>
        <div
          class="flex flex-col space-y-4 px-6 lg:px-12 lg:py-8 group-hover:-translate-y-1 transition-transform duration-200"
        >
          <img
            :src="useGetEmployeeImageUrl(item.image)"
            :alt="item.title"
            class="lg:-mt-32 rounded-xl w-full h-72 object-cover object-top mb-4"
          />
          <h3 class="text-center">Rendelési idő</h3>
          <ul class="pb-2">
            <li
